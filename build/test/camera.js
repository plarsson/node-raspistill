...
describe('camera', function () {
    ...
    it('should change options from one value to another', function (done) {
        var camera = new default_1.DefaultCamera({
            noFileSave: true
        });
        camera.setOptions({
        ...
        chai_1.expect(camera.getOption('noFileSave')).to.eq(false);
        done();
    });
    ...
    it('should exec camera in timelapse mode (file save)', function (done) {
        ...
        var i = 0;
        camera.timelapse(400, 2000, function (image) {
            chai_1.expect(image).to.be.instanceOf(Buffer);
            i++;
        }).then(function () {
            chai_1.expect(i).to.eq(5);
